...
class EMHDNNP(object):
	...
	def save_checkpoint(self, step):
		...
		print("Saving checkpoint file %s", checkpoint_file)
	...
	def load_data_to_scratch(self):
		...
		"""
		self.load_data()
		self.elements = np.unique(self.Z_data)[1:]
		...
		print("Number of training cases: ", self.num_train_cases)
		print("Number of validation cases: ", self.num_validation_cases)
		print("Number of test cases: ", self.num_test_cases)
		return
	...
	def get_train_batch(self):
		...
		batch_data = [batch_xyzs, batch_Zs, nn_pairs, nn_triples]
	...
	def fill_feed_dict(self, batch_data):
		...
		"""
		pl_list = [self.xyzs_pl, self.Zs_pl, self.nn_pairs_pl, self.nn_triples_pl]
		if self.train_charges:
			pl_list.append(self.coulomb_pairs_pl)
		pl_list.extend([self.num_atoms_pl, self.energy_pl, self.gradients_pl])
		if self.train_charges:
	...
	def train_prepare(self, restart=False):
		...
		with tf.Graph().as_default():
			...
			self.element_codes = tf.Variable(self.element_codes, trainable=True,
				dtype=self.tf_precision, name="element_modes")
	...
	def validation_step(self, step):
		...
		for i in [random.randint(0, num_atoms_epoch - 1) for _ in range(10)]:
			print("Forces label: ", validation_force_labels[i], " Forces output: ", validation_force_outputs[i])
		if self.train_charges:
			...
			for i in [random.randint(0, num_atoms_epoch - 1) for _ in range(10)]:
				print("Charge label: ", validation_charge_labels[i], " Charge output: ", validation_charge_outputs[i])
			print("MAE  Energy: ", np.mean(np.abs(validation_energy_errors)), " Forces: ",
				np.mean(np.abs(validation_force_errors)), " Charges: ", np.mean(np.abs(validation_charge_errors)))
			print("MSE  Energy: ", np.mean(validation_energy_errors), " Forces: ",
				np.mean(validation_force_errors), " Charges: ", np.mean(validation_charge_errors))
			print("RMSE  Energy: ", np.sqrt(np.mean(np.square(validation_energy_errors))), " Forces: ",
				np.sqrt(np.mean(np.square(validation_force_errors))), " Charges: ",
				np.sqrt(np.mean(np.square(validation_charge_errors))))
		else:
			print("MAE  Energy: ", np.mean(np.abs(validation_energy_errors)), " Forces: ",
		...
		self.print_epoch(step, duration, validation_loss, validation_energy_loss,
			validation_gradient_loss, validation_charge_loss, mode="validation")
	...
	def test_step(self, step):
		...
		for i in [random.randint(0, num_atoms_epoch - 1) for _ in range(10)]:
			print("Forces label: ", test_force_labels[i], " Forces output: ", test_force_labels[i])
		if self.train_charges:
			...
			for i in [random.randint(0, num_atoms_epoch - 1) for _ in range(10)]:
				print("Charge label: ", test_charge_labels[i], " Charge output: ", test_charge_outputs[i])
			print("MAE  Energy: ", np.mean(np.abs(test_energy_errors)), " Forces: ",
				np.mean(np.abs(test_force_errors)), " Charges: ", np.mean(np.abs(test_charge_errors)))
			print("MSE  Energy: ", np.mean(test_energy_errors), " Forces: ",
				np.mean(test_force_errors), " Charges: ", np.mean(test_charge_errors))
			print("RMSE  Energy: ", np.sqrt(np.mean(np.square(test_energy_errors))), " Forces: ",
				np.sqrt(np.mean(np.square(test_force_errors))), " Charges: ",
				np.sqrt(np.mean(np.square(test_charge_errors))))
		else:
			print("MAE  Energy: ", np.mean(np.abs(test_energy_errors)), " Forces: ",
		...
		self.print_epoch(step, duration, test_loss, test_energy_loss, test_gradient_loss,
			test_charge_loss, mode="test")
		return test_loss

	def compute_normalization(self):
		elements = self.elements.tolist()
		max_atomic_num = np.amax(self.Z_data)
	...
	def print_epoch(self, step, duration, loss, energy_loss, gradient_loss, charge_loss, mode="train"):
		if mode == "test":
			print("step: ", step, " duration: ", duration, " test loss: ", loss, " energy loss: ",
			energy_loss, " gradient loss: ", gradient_loss, " charge loss: ", charge_loss)
		elif mode == "validation":
			print("step: ", step, " duration: ", duration, " validation loss: ", loss, " energy loss: ",
			energy_loss, " gradient loss: ", gradient_loss, " charge loss: ", charge_loss)
		elif mode == "train":
			print("step: ", step, " duration: ", duration, " train loss: ", loss, " energy loss: ",
			energy_loss, " gradient loss: ", gradient_loss, " charge loss: ", charge_loss)
		return
